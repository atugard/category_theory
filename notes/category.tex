...
\begin{defn}
  ...
  \item For all morphisms $f:A \to B$ of $\mcC$ a unique morphism $Ff:FA \to FB$ of $\mcD$, such that:
    ...
    \end{itemize}
  \end{itemize}
  The definition for a contravariant functor is identical except that if $f:A \to B$ in $\mcC$ then $Ff: FB \to FA$ in $\mcD$, so that the composition requirement is
  \begin{center}
  $F(g \circ f) = Ff \circ Fg$, for $g: B \to C$.
    \end{center}
\end{defn}
